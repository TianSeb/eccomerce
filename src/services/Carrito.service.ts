import { cartDatabase } from "../repository/DbRepsitory";
...
export default class CarritoService implements ICarritoService {
    ...
    connection:any 
    constructor(){
        this.connection = cartDatabase
    }

    async createCart(): Promise<any> {
        return await this.connection.insert(new Carrito())
    }

    async deleteCartById(idCart:string): Promise<void> {
        return await this.connection.where('id',idCart).del()
    }

    async getCart(idCart:string): Promise<Array<any>> {
       if(idCart) return this.connection.where('id',idCart)

       return await this.connection
    }
    
    async getCartProductos(idCart:string): Promise<Array<Producto>> {
        const carrito : Carrito = await this.connection.where('id',idCart)
        return carrito.getProductos
    }
    
    async saveProductInCart(idCart:string,data:any): Promise<string> {
        const carrito = await this.connection.where('id',idCart)
        ...
        const prodSaved : any = await this.connection.insert(carrito)
